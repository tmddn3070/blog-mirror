...
// node_modules/.pnpm/swup@4.6.1/node_modules/swup/dist/Swup.modern.js
function i() {
  return i = Object.assign ? Object.assign.bind() : function(t) {
    for (var e = 1; e < arguments.length; e++) {
      var i2 = arguments[e];
      for (var s2 in i2) Object.prototype.hasOwnProperty.call(i2, s2) && (t[s2] = i2[s2]);
    }
    return t;
...
async function u(t, e = {}) {
  ...
  const { status: m2, url: g2 } = u2, w2 = await u2.text();
  if (500 === m2) throw this.hooks.call("fetch:error", n2, { status: m2, response: u2, url: g2 }), new c(`Server error: ${g2}`, { status: m2, url: g2 });
  if (!w2) throw new c(`Empty response: ${g2}`, { status: m2, url: g2 });
  const { url: f2 } = l.fromUrl(g2), v = { url: f2, html: w2 };
...
var S = class {
  ...
  parseCallArgs(t, e, i2, s2) {
    return e instanceof k || "object" != typeof e && "function" != typeof i2 ? [e, i2, s2] : [void 0, e, i2];
  }
...
var P = "transition";
var U = "animation";
async function C({ elements: t, selector: e }) {
  if (false === e && !t) return;
  let i2 = [];
  if (t) i2 = Array.from(t);
  else if (e && (i2 = m(e, document.body), !i2.length)) return void console.warn(`[swup] No elements found matching animationSelector \`${e}\``);
  const s2 = i2.map((t2) => function(t3) {
    const { type: e2, timeout: i3, propCount: s3 } = function(t4) {
      const e3 = window.getComputedStyle(t4), i4 = $(e3, `${P}Delay`), s4 = $(e3, `${P}Duration`), n2 = x(i4, s4), o2 = $(e3, `${U}Delay`), r2 = $(e3, `${U}Duration`), a2 = x(o2, r2), l2 = Math.max(n2, a2), h = l2 > 0 ? n2 > a2 ? P : U : null;
      return { type: h, timeout: l2, propCount: h ? h === P ? s4.length : r2.length : 0 };
    }(t3);
    return !(!e2 || !i3) && new Promise((n2) => {
      ...
      const l2 = () => {
        ...
        if (e3.target === t3) {
          if (!function(t4) {
            return [`${P}end`, `${U}end`].includes(t4.type);
          }(e3)) throw new Error("Not a transition or animation event.");
...
function x(t, e) {
  ...
  return Math.max(...e.map((e2, i2) => A(e2) + A(t[i2])));
}
function A(t) {
...
async function V(t, e = {}) {
  ...
  try {
    ...
    if (this.currentPageUrl = n(), t.history.popstate && this.classes.add("is-popstate"), t.animation.name && this.classes.add(`to-${s(t.animation.name)}`), t.animation.wait && await i3, t.done) return;
...
var T = async function(t, e) {
  if (t.done) return;
  t.advance(6);
  const { url: i2 } = e;
  this.isSameResolvedUrl(n(), i2) || (r(i2), this.currentPageUrl = n(), t.to.url = this.currentPageUrl), await this.hooks.call("content:replace", t, { page: e }, (t2, {}) => {
...
//# sourceMappingURL=chunk-KEBQA3AE.js.map
